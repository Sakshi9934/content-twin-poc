...
// A TRUE agent. Claude gets ALL FOUR tools and decides the flow itself:
//   - prefer Content Twins
//   - if no twin covers the question, fall back to the live pages on its own
// The useTwin flag only sets the preference; Claude still drives.
...
// ---- all four tools, always available ----
const TOOLS = [
  {
    name: 'list_twins',
    description:
...
function systemPrompt(preferTwin: boolean): string {
  const order = preferTwin
    ...
    : 'Use the live pages: call list_pages, then get_page on the most relevant page, and answer from it.';
  ...
  return (
    ...
    'Always cite the source URL you used. Begin your answer by stating whether you answered from a ' +
    'Content Twin or from the live page. If neither the twins nor the pages contain the answer, ' +
    'say you could not find relevant content.'
  );
}
  ...
  for (let step = 0; step < MAX_STEPS; step++) {
    const res = await fetch(ANTHROPIC_URL, {
      ...
      body: JSON.stringify({
        ...
        tools: TOOLS, // all four — Claude decides which to use
        messages,
      }),
    });
    ...
    const answer = data.content.filter((b): b is TextBlock => b.type === 'text').map((b) => b.text).join('\n').trim();
    return { answer, source: deriveSource(toolsUsed), toolsUsed };
  }

  return { answer: 'The agent could not complete the request within the step limit.', source: deriveSource(toolsUsed), toolsUsed };
}